...
def run_simulation(env, strategy, max_stations, model=None):
    ...
    while not done:
        ...
        # 3. 咱们的强化学习高维上帝视角
            ...
            # 结界 2：物理安检门 (获取环境真实的防爆仓掩码：最多2个订单)
            try:
            ...
            # 绝对不能放行 env_internal_mask（因为那会把订单发给已经断电的机器），
            # 只能退回到 energy_saving_mask，强行塞给开着的机器，让环境用死等惩罚它！
...
def main():
    print("="*80)
    print("📊 启动 [资源受限降维打击] 多算法横向对比实验 (双重结界修复版)")
    print("="*80)
    ...
    try:
        model = MaskablePPO.load(model_path, env=env)
        print(f"✅ 成功装载 AI 大脑: {os.path.basename(model_path)}")
    except Exception as e:
        print(f"⚠️ 警告: 尚未找到训练好的 AI 模型 ({model_path})。")
        print("请确保 train_agent_v1.py 至少跑出过一个 .zip 文件。现在仅展示传统算法图表。")
        model = None
    ...
    # 图表装饰
    plt.title("多目标排程降维打击测试：完工时间 vs 站台启用数 (双结界保护版)", fontsize=16, pad=15)
    plt.xlabel("允许启用的最大站台数量 (个)", fontsize=12)
    plt.ylabel("总完工耗时 Makespan (秒)", fontsize=12)
    plt.xticks(station_limits)
    plt.grid(True, alpha=0.3)
