...
class PickingEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        if self.current_step < self.total_orders:
            ...
            # 🌟 把它替换成这样（因为重工单子最大耗时可能接近 30000 秒，除以 30000 防止数据溢出让 AI 眼瞎）：
            ...
            # 特征A：Deadline 紧迫度 (当前时间/死线时间)，逼近 1.0 时 AI 会更加焦虑
    ...
    def step(self, action):
        ...
        if len(active_indices) > 0 and not is_new_machine:
            ...
            if actual_cost == np.min(active_costs):
                reward += 1.0  # 乖巧：发给了亮着的机器里最闲的
            else:
                reward -= 0.5  # 警告：发给了亮着但比较忙的，容易导致积压
                
        # 2. 温和的开机阻力 (替代粗暴的绝对惩罚)
        if is_new_machine:
            reward -= 2.0  # 微微刺痛一下，让它知道开机器要三思

        self.current_step += 1
        done = self.current_step >= self.total_orders
        
        # 3. 终局结算：废除断崖式 If-Else，改为【全时段线性平滑打分】！
        if done:
            makespan = np.max(self.station_workloads)
            active_machines = int(np.sum(self.station_active_status))
            
            # 评价指标 A：效率分 (基准假设最烂要 80000秒，每快 100 秒得 1 分)
            # 例如跑了 50000秒 -> 得 300分；跑了 70000秒 -> 得 100分。
            makespan_score = (80000.0 - makespan) / 100.0
            
            # 评价指标 B：环保分 (每省下一台机器，额外重奖 20 分)
